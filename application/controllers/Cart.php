...
class Cart extends CI_Controller
{
	...
	public function detail_kost($id3)
	{
    /* mengambil data berdasarkan id */
		$row = $this->Cart_model->get_by_id_front1($id3);

    /* melakukan pengecekan data, apabila ada maka akan ditampilkan */
		if ($row)
    {
      /* memanggil function dari masing2 model yang akan digunakan */
    	$this->data['kost_detail']     = $this->Cart_model->get_by_id_front1($id3);
      $this->data['Kost_lainnya']    = $this->Cart_model->get_all_random();

      $this->data['title'] 						= $row->nama_kost;

      /* memanggil view yang telah disiapkan dan passing data dari model ke view*/
			$this->load->view('front/cart/detail_kost', $this->data);
		}
		else
    {
			$this->session->set_flashdata('message', '<div class="alert alert-dismissible alert-danger">
        <button type="button" class="close" data-dismiss="alert">&times;</button>Kost tidak ditemukan</b></div>');
      redirect(base_url());
    }
	}
	...
	public function getJamMulai()
	{
		...
		if ($tanggal === FALSE || $kost_id === FALSE) {
			echo json_encode(array());
			die();
		}

		$list_jam_mulai_terpakai = $this->Transaksi_detail_model->get_jam_mulai_terpakai($tanggal, $kost_id);
