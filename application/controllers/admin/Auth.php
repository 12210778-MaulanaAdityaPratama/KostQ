...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		// Cek super bukan
		elseif (!$this->ion_auth->is_AdminKost()) {
	...
	public function login()
	{
		...
		if ($this->form_validation->run() == FALSE || !isset($response['success']) || $response['success'] <> TRUE) {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
				...
				<i class="ace-icon fa fa-bullhorn green"></i> Email atau Passwoard salah, mohon diperiksa lagi
				</div></div>', $this->ion_auth->errors(''));
	...
	public function activate($id, $code = FALSE)
	{
		...
		} else if ($this->ion_auth->is_AdminKost()) {
	...
	public function deactivate($id = NULL)
	{
		...
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_AdminKost()) {
	...
	public function edit_user($id)
	{
		...
		// Cek hak akses ubah password user lain (Hanya AdminKost yang dibolehkan)
		if (!$this->ion_auth->logged_in() || (!$this->ion_auth->is_AdminKost() && !($this->ion_auth->user()->row()->id == $id))) {
			redirect('admin/dashboard', 'refresh');
		}
