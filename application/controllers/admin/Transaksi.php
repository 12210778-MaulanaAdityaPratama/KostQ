...
class Transaksi extends CI_Controller
{
  ...
  public function print($id)
  {
    ...
    elseif($this->ion_auth->is_AdminKost() && $this->session->userdata('company') != $row->company)
    {
      $this->session->set_flashdata('message', "<script>alert('Data tidak ditemukan');</script>");
      redirect(site_url('transaksi'));
    }
    elseif($this->ion_auth->is_admin() && $this->session->userdata('cabang') != $row->cabang)
    {
      ...
      {
        if($this->ion_auth->is_AdminKost())
        {
          $data['cek_data']   = $this->Transaksi_model->get_by_id_print_AdminKost($id);
          $data['transaksi_detail'] = $this->Transaksi_model->get_by_id_print_detail_AdminKost($id);
        }
