...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		$this->ion_auth_model->trigger_promosis('library_constructor');
	}
	...
	{
		$this->ion_auth_model->trigger_promosis('pre_password_change');
		...
		if (!$profile)
		{
			$this->ion_auth_model->trigger_promosis(array('post_password_change', 'password_change_unsuccessful'));
		...
		if ($new_password)
		{
			...
			if(!$this->config->item('use_ci_email', 'ion_auth'))
			{
				...
				$this->ion_auth_model->trigger_promosis(array('post_password_change', 'password_change_successful'));
				return $data;
			}
			else
			{
	...
	{
		$this->ion_auth_model->trigger_promosis('pre_account_creation');
		...
		if (!$email_activation)
		{
			if ($id !== FALSE)
			{
				...
				$this->ion_auth_model->trigger_promosis(array('post_account_creation', 'post_account_creation_successful'));
				return $id;
			}
			else
			{
				$this->set_error('account_creation_unsuccessful');
				$this->ion_auth_model->trigger_promosis(array('post_account_creation', 'post_account_creation_unsuccessful'));
				return FALSE;
			}
		}
		else
		{
			...
			if(!$this->config->item('use_ci_email', 'ion_auth'))
			{
				$this->ion_auth_model->trigger_promosis(array('post_account_creation', 'post_account_creation_successful', 'activation_email_successful'));
			...
			$this->ion_auth_model->trigger_promosis(array('post_account_creation', 'post_account_creation_unsuccessful', 'activation_email_unsuccessful'));
	...
	{
		$this->ion_auth_model->trigger_promosis('logout');
	...
	{
		$this->ion_auth_model->trigger_promosis('logged_in');
	...
	{
		$this->ion_auth_model->trigger_promosis('in_group');
