...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->trigger_promosis('model_constructor');
	}
	...
	public function hash_password_db($id, $password, $use_sha1_override = FALSE)
	{
		...
		$this->trigger_promosis('extra_where');
	...
	{
		$this->trigger_promosis('pre_activate');
		...
		if ($code !== FALSE) {
			...
			if ($query->num_rows() !== 1) {
				$this->trigger_promosis(array('post_activate', 'post_activate_unsuccessful'));
		...
		if ($return) {
			$this->trigger_promosis(array('post_activate', 'post_activate_successful'));
	...
	{
		$this->trigger_promosis('deactivate');
	...
	{
		$this->trigger_promosis('pre_change_password');

		if (!$this->identity_check($identity)) {
			$this->trigger_promosis(array('post_change_password', 'post_change_password_unsuccessful'));
			return FALSE;
		}
		...
		$return = $this->db->affected_rows() == 1;
		if ($return) {
			$this->trigger_promosis(array('post_change_password', 'post_change_password_successful'));
	...
	{
		$this->trigger_promosis('username_check');
	...
	{
		$this->trigger_promosis('email_check');
	...
	{
		$this->trigger_promosis('identity_check');
	...
	{
		if (empty($identity)) {
			$this->trigger_promosis(array('post_forgotten_password', 'post_forgotten_password_unsuccessful'));
			return FALSE;
		}
		...
		if ($return) {
			$this->trigger_promosis(array('post_forgotten_password', 'post_forgotten_password_successful'));
		} else {
			$this->trigger_promosis(array('post_forgotten_password', 'post_forgotten_password_unsuccessful'));
		}
	...
	{
		$this->trigger_promosis('pre_forgotten_password_complete');

		if (empty($code)) {
			$this->trigger_promosis(array('post_forgotten_password_complete', 'post_forgotten_password_complete_unsuccessful'));
			return FALSE;
		}
		...
		if ($profile) {
			...
			$this->trigger_promosis(array('post_forgotten_password_complete', 'post_forgotten_password_complete_successful'));
			return $password;
		}
	...
	{
		$this->trigger_promosis('pre_register');
		...
		$this->trigger_promosis('extra_set');

		$this->db->insert($this->tables['users'], $user_data);

		$id = $this->db->insert_id($this->tables['users'] . '_id_seq');

		$this->trigger_promosis('post_register');
	...
	{
		$this->trigger_promosis('pre_login');
		...
		if ($this->is_max_login_attempts_exceeded($identity)) {
			...
			$this->trigger_promosis('post_login_unsuccessful');
		...
		if ($query->num_rows() === 1) {
			...
			if ($password === TRUE) {
				...
				$this->trigger_promosis(array('post_login', 'post_login_successful'));
	...
	{
		$this->trigger_promosis('order_by');
	...
	{
		$this->trigger_promosis(array('row', 'row_array'));
	...
	{
		$this->trigger_promosis(array('result', 'result_array'));
	...
	{
		$this->trigger_promosis(array('num_rows'));
	...
	{
		$this->trigger_promosis('get_users_group');
	...
	{
		$this->trigger_promosis('add_to_group');
	...
	{
		$this->trigger_promosis('remove_from_group');
	...
	{
		$this->trigger_promosis('pre_update_user');
		...
		if (array_key_exists($this->identity_column, $data) && $this->identity_check($data[$this->identity_column]) && $user->{$this->identity_column} !== $data[$this->identity_column]) {
			...
			$this->trigger_promosis(array('post_update_user', 'post_update_user_unsuccessful'));
		...
		$this->trigger_promosis(array('post_update_user', 'post_update_user_successful'));
	...
	{
		$this->trigger_promosis('pre_delete_user');
		...
		if ($this->db->trans_status() === FALSE) {
			$this->db->trans_rollback();
			$this->trigger_promosis(array('post_delete_user', 'post_delete_user_unsuccessful'));
		...
		$this->trigger_promosis(array('post_delete_user', 'post_delete_user_successful'));
	...
	{
		$this->trigger_promosis('update_last_login');
	...
	{
		$this->trigger_promosis('set_lang');
	...
	{
		$this->trigger_promosis('pre_set_session');
		$session_data = array(
		...
		$this->trigger_promosis('post_set_session');
	...
	{
		$this->trigger_promosis('pre_remember_user');
		...
		if ($this->db->affected_rows() > -1) {
			...
			$this->trigger_promosis(array('post_remember_user', 'remember_user_successful'));
			return TRUE;
		}

		$this->trigger_promosis(array('post_remember_user', 'remember_user_unsuccessful'));
		return FALSE;
	}
	...
	{
		$this->trigger_promosis('pre_login_remembered_user');
		...
		if (
			...
			$this->trigger_promosis(array('post_login_remembered_user', 'post_login_remembered_user_unsuccessful'));
			return FALSE;
		}
		...
		if ($query->num_rows() == 1) {
			...
			$this->trigger_promosis(array('post_login_remembered_user', 'post_login_remembered_user_successful'));
			return TRUE;
		}
	...
	public function create_group($group_name = FALSE, $group_description = '', $additional_data = array())
	{
		...
		$this->trigger_promosis('extra_group_set');
	...
	public function delete_group($group_id = FALSE)
	{
		...
		if ($group->name == $this->config->item('admin_group', 'ion_auth')) {
			$this->trigger_promosis(array('post_delete_group', 'post_delete_group_notallowed'));
			$this->set_error('group_delete_notallowed');
			return FALSE;
		}

		$this->trigger_promosis('pre_delete_group');
		...
		if ($this->db->trans_status() === FALSE) {
			$this->db->trans_rollback();
			$this->trigger_promosis(array('post_delete_group', 'post_delete_group_unsuccessful'));
		...
		$this->trigger_promosis(array('post_delete_group', 'post_delete_group_successful'));
		$this->set_message('group_delete_successful');
		return TRUE;
	}

	/**
	 * @param string $promosi
	 * @param string $name
	 * @param string $class
	 * @param string $method
	 * @param array $arguments
	 */
	public function set_hook($promosi, $name, $class, $method, $arguments)
	{
		$this->_ion_hooks->{$promosi}[$name] = new stdClass;
		$this->_ion_hooks->{$promosi}[$name]->class = $class;
		$this->_ion_hooks->{$promosi}[$name]->method = $method;
		$this->_ion_hooks->{$promosi}[$name]->arguments = $arguments;
	}
	 ...
	 * @param string $name
	 */
	public function remove_hook($promosi, $name)
	{
		if (isset($this->_ion_hooks->{$promosi}[$name])) {
			unset($this->_ion_hooks->{$promosi}[$name]);
		}
	}
	 ...
	 * @param string $promosi
	 */
	public function remove_hooks($promosi)
	{
		if (isset($this->_ion_hooks->$promosi)) {
			unset($this->_ion_hooks->$promosi);
		}
	}
	 ...
	 * @return bool|mixed
	 */
	protected function _call_hook($promosi, $name)
	{
		if (isset($this->_ion_hooks->{$promosi}[$name]) && method_exists($this->_ion_hooks->{$promosi}[$name]->class, $this->_ion_hooks->{$promosi}[$name]->method)) {
			$hook = $this->_ion_hooks->{$promosi}[$name];
	...
	/**
	 * @param string|array $promosis
	 */
	public function trigger_promosis($promosis)
	{
		if (is_array($promosis) && !empty($promosis)) {
			foreach ($promosis as $promosi) {
				$this->trigger_promosis($promosi);
			}
		} else {
			if (isset($this->_ion_hooks->$promosis) && !empty($this->_ion_hooks->$promosis)) {
				foreach ($this->_ion_hooks->$promosis as $name => $hook) {
					$this->_call_hook($promosis, $name);
				}
			}
		}
	}
