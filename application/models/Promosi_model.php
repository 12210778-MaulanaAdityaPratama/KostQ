...
class Promosi_model extends CI_Model
{
  ...
  public $id    = 'id_promosi';
  public $order = 'DESC';

  function get_all()
  {
    $this->db->join('kategori', 'promosi.kategori = kategori.id_kategori', 'LEFT');
  ...
  function get_all_promosi_sidebar()
  {
    $this->db->limit(5);
  ...
  {
    $this->db->where('slug_promosi', $id);
    return $this->db->get($this->table)->row();
  }

  function cek_kategori($id)
  {
    $this->db->join('kategori', 'promosi.kategori = kategori.id_kategori');
  ...
  // get all
  function get_cari_promosi()
  {
    $cari_promosi = $this->input->post('cari_promosi');

    $this->db->like('nama_promosi', $cari_promosi);
    return $this->db->get($this->table)->result();
  }
