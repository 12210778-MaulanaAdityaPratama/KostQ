	...
	<div class="row">
		<div class="col-md-8"><h1>Kategori: <?php echo ucfirst($this->uri->segment(3)) ?></h1><hr>
			<?php foreach($kategori_data as $kategori){ ?>
				<h2><a href="<?php echo base_url('promosi/').$kategori->slug_promosi ?>"><?php echo $kategori->nama_promosi ?></a></h2>
				<a href="<?php echo base_url("promosi/$kategori->slug_promosi ") ?>">
					<?php
					if(empty($kategori->foto)) {echo "<img class='img-responsive' src='".base_url()."assets/images/no_image_thumb.png'>";}
					else { echo " <img class='img-responsive' src='".base_url()."assets/images/promosi/".$kategori->foto.'_thumb'.$kategori->foto_type."'> ";}
					?>
				</a>
				<p>
				...
				<a class="btn btn-sm btn-primary" href="<?php echo base_url("promosi/$kategori->slug_promosi ") ?>">Selengkapnya <i class="fa fa-angle-right"></i></a>
			<?php } ?>
